...
// revised 2017-1-6
template <typename Dtype>
void Blob<Dtype>::twobit_data(Phase phase){
  ...
  case SyncedMemory::HEAD_AT_CPU:
    {
	    caffe_cpu_twobit<Dtype>(this->count(), this->cpu_data(), this->mutable_cpu_binary());
	    alpha = caffe_cpu_dot(this->count(), this->cpu_binary(), this->cpu_data());
	    alpha /= caffe_cpu_dot(this->count(), this->cpu_binary(), this->cpu_binary());
	    caffe_cpu_scale(this->count(), alpha, this->cpu_binary(), this->mutable_cpu_binary());
	    // this->set_alpha(alpha);
...
#ifndef CPU_ONLY
    {
      caffe_gpu_twobit<Dtype>(this->count(), this->gpu_data(), this->mutable_gpu_binary());
	    Dtype* pa = new Dtype(0);
	    caffe_gpu_dot(this->count(), this->gpu_binary(), this->gpu_data(), pa);
	    Dtype* pb = new Dtype(0);
	    caffe_gpu_dot(this->count(), this->gpu_binary(), this->gpu_binary(), pb);
...
template <typename Dtype>
void Blob<Dtype>::TwobitFromProto(const BlobProto& proto, bool reshape) {
  if (reshape) {
    vector<int> shape;
    if (proto.has_num() || proto.has_channels() ||
        proto.has_height() || proto.has_width()) {
  ...
  // std::cout << "----------- n ----------" << std::endl;
  Dtype* data_vec = mutable_cpu_binary();
  if(proto.twobit_data_size() > 0){
    CHECK_EQ((count_-1)/16+1, proto.twobit_data_size());
    for(int i=0; i<proto.twobit_data_size(); i++){
      int n = proto.twobit_data(i);
      //if(i<20){
        //std::cout << n << " ";
      //}
      for(int j=0; j<16 && i*16+j<count_; j++){
...
template <>
void Blob<float>::TwobitToProto(BlobProto* proto) const {
  proto->clear_shape();
  for (int i = 0; i < shape_.size(); ++i) {
    proto->mutable_shape()->add_dim(shape_[i]);
  }
  proto->clear_twobit_data();
  const float alpha = this->get_alpha();
  proto->set_alpha(alpha);
...
template <>
void Blob<double>::TwobitToProto(BlobProto* proto) const {
  proto->clear_shape();
  for (int i = 0; i < shape_.size(); ++i) {
    proto->mutable_shape()->add_dim(shape_[i]);
  }
