...
template<>
void caffe_cpu_twobit<int>(const int N, const int* X, int* Y){
// NOT IMPLEMENTED
}

template<>
void caffe_cpu_twobit<unsigned>(const int N, const unsigned* X, unsigned* Y){
// NOT IMPLEMENTED
}

template<>
void caffe_cpu_twobit<float>(const int N, const float* X, float* Y){
	for(int i=0; i<N; i++){
		float x = X[i];
    Y[i] = -2 * (x < -1) + -1 * (x >= -1&&x <= 0) + 1 * (x > 0&&x <= 1)  + 2 * (x > 1) ;
	}
}

template<>
void caffe_cpu_twobit<double>(const int N, const double* X, double* Y){
	for(int i=0; i<N; i++){
		double x = X[i];
    Y[i] = -2 * (x < -1) + -1 * (x >= -1&&x <= 0) + 1 * (x > 0&&x <= 1)  + 2 * (x > 1) ;
	}
